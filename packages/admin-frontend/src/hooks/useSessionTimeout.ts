/**
 * ★ 한줄전단 슈퍼관리자 전용 세션 타임아웃 훅
 *
 * 매장 사장님 frontend와 분리:
 * - localStorage 키: admin_token (매장 사장님은 flyer_token)
 * - 서버 세션 연장 API: /api/flyer/super/session-check (슈퍼관리자 전용)
 */
...
export function useSessionTimeout({ onLogout }: UseSessionTimeoutOptions): UseSessionTimeoutReturn {
  ...
  const extendSession = useCallback(() => {
    ...
    try {
      ...
      if (token) {
        fetch('/api/flyer/super/session-check', {
          method: 'POST',
          headers: {
