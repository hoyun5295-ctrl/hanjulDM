...
// ══════════════════════════════════════════
// POST /auto-images는 2026-08-21 폐지 — 소스였던 네이버 쇼핑 검색 API가 2026-07-31 서비스 종료(대체 없음).
// 무인 자동 부착 축은 복구 불가. 이미지 부착은 후보 제시 + 사람 탭 1회 확정 구조로 전환 —
// 화면(FlyerComposerPage 사진 고르기)이 catalog/search-image(후보) + catalog/select-image(확정)를 쓴다.
// 경위·이관 기한(2027-06-30 API HUB) = 15번 문서 §8 · CT-F17 헤더.
// ══════════════════════════════════════════
